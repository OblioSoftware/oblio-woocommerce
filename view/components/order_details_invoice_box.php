...
$order = new OblioSoftware\Order(method_exists($post, 'get_id') ? $post->get_id() : $post->ID);
...
$displayDocument = function ( $post, $options = [] ) use ( $wpdb, $order ) {
	...
	if ( ! $link || $lastInvoice == (method_exists($post, 'get_id') ? $post->get_id() : $post->ID) || $options['docType'] === 'proforma' ) {
		$hidden = $link ? '' : 'hidden';
		echo sprintf(
