...
$displayDocument = function ( $post, $options = [] ) use ( $wpdb, $order ) {
	...
	switch ( $options['docType'] ) {
		case 'proforma':
			$series_name = get_option( 'oblio_series_name_proforma' );
			break;
		default:
	...
	// Set correct table and field names
	...
	// Corrected SQL query to get last invoice for a given series
	$sql = $wpdb->prepare( "
	...
	$lastInvoice = $wpdb->get_var( $sql ); // Run regardless of $link
	...
	// Move the script inside the function
	?>
